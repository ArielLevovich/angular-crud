...
import { EmployeesHttpClient } from '../employees-http-client';
...
export class EmployeesListComponent implements AfterViewInit {
    ...
    studentsHttpClient: EmployeesHttpClient | null;
    students: Employee[] = [];
    sortColumn = 'phone';
    ...
    editStudentId = -1;
    isAdding = false;
    selectedStudents: Set<Employee> = new Set<Employee>();
    ...
    employeeIdFormControl = new FormControl('', { updateOn: 'blur', validators: [Validators.required],
                                                    asyncValidators: [this.studentSchoolIdValidator()] });
    employeeNameFormControl = new FormControl('', [Validators.required]);
    employeePhoneFormControl = new FormControl('', [Validators.required]);
    employeeEmailFormControl = new FormControl('', [Validators.required, Validators.email]);
     ...
     */
    ngAfterViewInit() {
        this.studentsHttpClient = new EmployeesHttpClient();
     ...
     */
    studentToggle(student: Employee): void {
     ...
     *
     * @param studentId the employeeId
     */
    ...
    onEditClick(studentId: number): void {
        ...
        const student: Employee = this.students.find(s => s.employeeId === studentId);
        ...
        this.employeeIdFormControl.setValue(student.employee);
        this.employeeNameFormControl.setValue(student.name);
        this.employeePhoneFormControl.setValue(student.phone);
        this.employeeEmailFormControl.setValue(student.email);
    }
    ...
    isUpdateDisabled(): boolean {
        ...
        if (this.employeeIdFormControl.invalid || this.employeeNameFormControl.invalid ||
            this.employeePhoneFormControl.invalid || this.employeeEmailFormControl.invalid) {
            return true;
        }
    ...
    onUpdateClick(studentId: number): void {
        ...
        const student: Employee = {
            employeeId: this.editStudentId,
            employee: `${this.employeeIdFormControl.value}`,
            name: `${this.employeeNameFormControl.value}`,
            phone: `${this.employeePhoneFormControl.value}`,
            email: `${this.employeeEmailFormControl.value}`
        };
    ...
    onAddStudentClick(): void {
        ...
        this.employeePhoneFormControl.reset();
        this.employeeEmailFormControl.reset();
    }
    ...
    onDeleteSelectedClick(): void {
        ...
        modalRef.componentInstance.message = 'Are you sure you want to delete the selected employees?';

        modalRef.result.then((confirmedDelete: boolean) => {
            if (confirmedDelete) {
                this.isProcessing = true;
                const studentIds: number[] = new Array(this.selectedStudents.size);
                this.selectedStudents.forEach(s => studentIds.push(s.employeeId));
     ...
     *
     * @param studentId the employeeId to check
     */
     ...
     *
     * @param studentId the employeeId to add to the set of expanded students
     */
     ...
     *
     * @param studentId the employeeId to remove from the set of expanded students
     */
